...
describe('LOGIN FUNCTIONALITY', () => {
  ...
  it('TC-029 should verify that user can login as NEW USER', () => {
  ...
  it('TC-028 should verify that user can login as ADMIN', () => {
    LoginPage.login(user.admin.email, user.admin.password);
    TopMenuPage.userAvatarName.waitForDisplayed({ timeout: waitTime.WAIT_TIME_MEDIUM });
    expect(ProfilePage.getLoginConfirmation()).eq(user.admin.firstName + ' ' + user.admin.lastName);
  });

  it('TC-030 should verify that user can login as LEARNER', () => {
  ...
  it('TC-031 should verify that user can login as STUDENT', () => {
