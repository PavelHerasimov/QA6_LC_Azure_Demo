...
import { newRole } from '../data/settingsPasswordData';
...
describe('CORRECT FUNCTIONALITY', () => {
  ...
  it('TC-003-017 Old password eye icon is works', () => {
  ...
  it('TC-003-018 New password eye icon is works', () => {
  ...
  it('TC-003-019 Confirm new password eye icon is works', () => {
...
describe('NEGATIVE FUNCTIONALITY', () => {
  it('TC-003-013 Update password button disabled until all fields are filled in (Confirm new password = empty)', () => {
    SettingsPasswordPage.updatePasswordNegative(newRole.oldPassword, newRole.newPassword, newRole.confirmNewPasswordEmpty);
    expect(SettingsPasswordPage.updatePasswordButton.isEnabled()).false;
  });

  it('TC-003-014 Update password button disabled until all fields are filled in (new password = empty)', () => {
    browser.refresh();
    SettingsPasswordPage.updatePasswordNegative(newRole.oldPassword, newRole.newPasswordEmpty, newRole.confirmNewPassword);
    expect(SettingsPasswordPage.updatePasswordButton.isEnabled()).false;
  });

  it('TC-003-015 Update password button disabled until all fields are filled in (old password = empty)', () => {
    browser.refresh();
    SettingsPasswordPage.updatePasswordNegative(newRole.oldPasswordEmpty, newRole.newPasswordEmpty, newRole.confirmNewPassword);
    expect(SettingsPasswordPage.updatePasswordButton.isEnabled()).false;
  });

  it('TC-003-020 Wrong old password: pop-up error message will appear ', () => {
    browser.refresh();
  ...
  it('TC-003-021 Wrong old password: pop-up error message = User Settings Update: Error', () => {
    browser.refresh();
    SettingsPasswordPage.updatePassword(newRole.oldPasswordIncorrect, newRole.newPassword);
    SettingsPasswordPage.popUpWrongOldPwrd.waitForDisplayed(WAIT_TIME_MEDIUM);
  ...
  it('TC-003-022 pop-up error window closed after refresh', () => {
    browser.refresh();
    expect(SettingsPasswordPage.popUpWrongOldPwrd.isDisplayed()).false;
  });

  it('TC-003-023  pop-up error window can be closed by clicking icon cross', () => {
    SettingsPasswordPage.updatePassword(newRole.oldPasswordIncorrect, newRole.newPassword);
  ...
  it('TC-003-024 "New password mismatch: error message is appears', () => {
    browser.refresh();
  ...
  it('TC-003-025 "New password mismatch: error message = New passwords do not match', () => {
    browser.refresh();
    SettingsPasswordPage.updatePasswordNegative(newRole.oldPassword, newRole.newPassword, newRole.confirmNewPasswordIncorrect);
    expect(SettingsPasswordPage.errorMsgPasswordNotMatch.getText()).eq(expected.settingsPasswordData['TC-003-025-errorMsg']);
  });

  it('TC-003-030 User can not log in using the old password', () => {
    browser.refresh();
