...
import { userUpdateRole } from '../../../regression/user/helpers/updateUserRoleByAxios';
import { roles, admin } from '../../../regression/user/data/settingsPasswordData';
...
describe('NEW USER CHANGE PASSWORD', () => {
  ...
  it('TC-003-028a User stays logged in after password has been changed', () => {
    SettingsPasswordPage.updatePassword(user.password, user.newPassword);
    expect(ProfilePage.dropDownUserMenu.isDisplayed()).true;
  });

  it('TC-003-029a User can log in using the new password', () => {
    ProfilePage.logout();
...
describe('LEARNER CHANGE PASSWORD', () => {
  ...
  it('TC-003-028b User stays logged in after password has been changed', () => {
    SettingsPasswordPage.updatePassword(user.newPassword, user.password);
    expect(ProfilePage.dropDownUserMenu.isDisplayed()).true;
  });

  it('TC-003-029b User can log in using the new password', () => {
    ProfilePage.logout();
...
describe('STUDENT CHANGE PASSWORD', () => {
  ...
  it('TC-003-028c User stays logged in after password has been changed', () => {
    SettingsPasswordPage.updatePassword(user.password, user.newPassword);
    expect(ProfilePage.dropDownUserMenu.isDisplayed()).true;
  });

  it('TC-003-029c User can log in using the new password', () => {
    ProfilePage.logout();
    LoginPage.login(user.email, user.newPassword);
...
describe('ADMIN CHANGE PASSWORD', () => {
  ...
  it('TC-003-028d User stays logged in after password has been changed', () => {
    SettingsPasswordPage.updatePassword(admin.oldPassword, admin.newPassword);
    expect(ProfilePage.dropDownUserMenu.isDisplayed()).true;
  });

  it('TC-003-029d User can log in using the new password', () => {
    ProfilePage.logout();
  ...
  it('Return old password for Admin', () => {
    ProfilePage.dropDownUserMenu.click();
    ProfilePage.settingsLink.waitForClickable();
    ProfilePage.settingsLink.click();
    SettingsPasswordPage.passwordTab.click();
